...
function deriveChange(name: string, arg: string): Change | null {
  ...
  if (lower === 'bash') {
    if (/gh\s+pr\s+create/.test(arg)) return { t: 'pr', x: 'PR opened' }
    if (/git\s+commit/.test(arg)) return { t: 'commit', x: arg.replace(/^.*git\s+commit\s*/, 'commit ') }
    if (/gh\s+issue\s+create/.test(arg)) return { t: 'pr', x: 'Issue opened' }
    if (/gh\s+issue\s+edit/.test(arg)) return { t: 'label', x: arg }
  }
  return null
}
...
export function runClaude(
  ...
  return new Promise<RunResult>((resolve) => {
      ...
      if (evt.type === 'assistant' && evt.message?.content) {
        ...
        for (const block of evt.message.content) {
          if (block.type === 'tool_result') {
            ...
            if (text) push({ role: 'result', text, err: block.is_error === true })
          }
        }
      } else if (evt.type === 'result') {
        if (typeof evt.result === 'string') finalSummary = evt.result
        if (typeof evt.total_cost_usd === 'number') costUsd = +evt.total_cost_usd.toFixed(4)
    ...
    const finish = (code: number | null): void => {
      ...
      if (!summary) summary = 'Completed — see transcript for details.'
      resolve({
...
export function createRunningRun(routineId: string, prompt: string, dir: string, trigger: Run['trigger']): Run {
  return {
