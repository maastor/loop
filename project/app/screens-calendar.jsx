...
function dayKey(d) { d = new Date(d); return `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`; }

function CalendarScreen({ routines, runs, now, nav }) {
  const [mode, setMode] = React.useState('month');
  const [cursor, setCursor] = React.useState(new Date(now.getFullYear(), now.getMonth(), now.getDate()));
  ...
  const goToday = () => { const t = new Date(now.getFullYear(), now.getMonth(), now.getDate()); setCursor(t); setSelected(dayKey(t)); };
  ...
  const selDate = (() => { const [y, m, d] = selected.split('-').map(Number); return new Date(y, m, d); })();
    ...
    <div className="screen" data-screen-label="Calendar">
      ...
      <div className="cal-layout">
        <div className="panel cal-panel">
          <div className="cal-nav">
            ...
            <div className="cal-nav-btns">
              <Btn ghost small onClick={goToday}>Today</Btn>
          ...
          {mode === 'month' ? (
            <div>
              <div className="cal-dow mono">
                {calUtil.DAY_NAMES.map((d) => <div key={d}>{d}</div>)}
              </div>
                    ...
                    <button type="button" key={i}
                      ...
                      {dayRuns.length > 0 ? (
                        <span className="cal-dots">
                          {dayRuns.slice(0, 4).map((r, j) => <StatusDot key={j} status={r.status} size={5} />)}
                          {dayRuns.length > 4 ? <span className="cal-more mono">+{dayRuns.length - 4}</span> : null}
                        </span>
                      ) : null}
                      {failed > 0 ? <span className="cal-fail mono">{failed} failed</span> : running ? <span className="cal-running mono">running</span> : null}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="week-grid">
              {weekDays.map((d) => {
                ...
                const dayRuns = runsByDay[k] || [];
                return (
                  <div key={k} className={'week-col' + (k === todayKey ? ' today' : '') + (k === selected ? ' sel' : '')}
                    onClick={() => setSelected(k)}>
                    ...
                    <div className="week-col-body">
                      {dayRuns.length === 0 ? <span className="dim mono week-empty">—</span> : dayRuns.map((r) => (
                        <button type="button" key={r.id} className="week-chip"
                          onClick={(e) => { e.stopPropagation(); nav({ screen: 'run', runId: r.id, from: { screen: 'calendar' } }); }}>
        ...
        <div className="panel cal-side">
          ...
          {selRuns.length === 0 ? (
            <div className="dim" style={{ fontSize: 13, padding: '10px 2px' }}>No runs this day.</div>
          ) : (
            <div className="run-list">
              {selRuns.map((run) => (
                <div key={run.id} className="run-row compact" onClick={() => nav({ screen: 'run', runId: run.id, from: { screen: 'calendar' } })}>
                  <StatusDot status={run.status} />
                  <div className="cal-side-main">
                    <div className="cal-side-name">{routineName(run.routineId)}</div>
                    <div className="mono dim" style={{ fontSize: 11 }}>{calUtil.fmtTime(run.start)} · {calUtil.fmtDur(run.durationSec)}</div>
                  </div>
