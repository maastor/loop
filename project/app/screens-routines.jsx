...
function NextRunLabel({ routine, now }) {
  ...
  return <span className="mono">{routinesUtil.relUntil(next, now)} · {routinesUtil.fmtTime(next)}</span>;
}

function LastRunCell({ run, now, onOpen }) {
  if (!run) return <span className="dim mono">never ran</span>;
  return (
    <button type="button" className="lastrun-link" onClick={(e) => { e.stopPropagation(); onOpen(run.id); }}>
      <StatusDot status={run.status} size={6} />
      <span className="mono">{run.status === 'running' ? 'running now' : routinesUtil.relTime(run.start, now)}</span>
    </button>
  );
}
    ...
    <div className="spark" aria-hidden="true">
      {last.map((r, i) => (
        <span key={i} className="spark-bar" style={{
          background: r.status === 'failed' ? 'var(--red)' : r.status === 'running' ? 'var(--accent)' : 'var(--green-dim)',
  ...
  return (
    <div className="row-list">
      {routines.map((r) => {
        ...
        return (
          <div key={r.id} className={'rt-row' + (r.enabled ? '' : ' off')} onClick={() => onOpen(r.id)}>
            <Toggle value={r.enabled} onChange={() => onToggle(r.id)} small />
            <div className="rt-row-main">
              <div className="rt-row-name">{r.name}</div>
              <div className="rt-row-meta mono">{routinesUtil.describeSchedule(r.schedule)} · {r.dir}</div>
            </div>
            <RunSpark runs={runs} />
            ...
            <Btn ghost small icon="play" title="Run now" onClick={(e) => { e.stopPropagation(); onRunNow(r.id); }} />
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="card-grid">
      {routines.map((r) => {
        ...
        const last = runs[0];
        return (
          <div key={r.id} className={'rt-card' + (r.enabled ? '' : ' off')} onClick={() => onOpen(r.id)}>
        ...
        {routines.map((r) => {
          ...
          return (
            <tr key={r.id} className={r.enabled ? '' : 'off'} onClick={() => onOpen(r.id)}>
              <td><Toggle value={r.enabled} onChange={() => onToggle(r.id)} small /></td>
              <td>
              ...
              <td><ModelChip model={r.model} /></td>
              <td><LastRunCell run={runs[0]} now={now} onOpen={onOpenRun} /></td>
              <td><NextRunLabel routine={r} now={now} /></td>
              <td><Btn ghost small icon="play" title="Run now" onClick={(e) => { e.stopPropagation(); onRunNow(r.id); }} /></td>
            </tr>
          );
        })}
      </tbody>
...
function RoutinesScreen({ routines, runs, now, layout, nav, onToggle, onRunNow, onNew }) {
  ...
  const Variant = { rows: RoutineRows, cards: RoutineCards, table: RoutineTable }[layout] || RoutineRows;
  ...
  return (
    <div className="screen" data-screen-label="Routines list">
      <ScreenHead title="Routines" sub={`${active} active · ${routines.length - active} paused`}>
        <Btn primary icon="plus" onClick={onNew}>New routine</Btn>
      </ScreenHead>
      {routines.length === 0 ? (
        <EmptyState icon="routines" title="No routines yet"
          body="A routine is a prompt Claude Code runs on a schedule — triage issues every morning, audit dependencies nightly.">
          <Btn primary icon="plus" onClick={onNew}>Create your first routine</Btn>
        </EmptyState>
      ) : (
        <Variant routines={routines} runsByRoutine={runsByRoutine} now={now}
  ...
  return (
    <div className="screen" data-screen-label="Routine detail">
      ...
      <ScreenHead title={routine.name}
        ...
        <Btn ghost icon="play" onClick={() => onRunNow(routine.id)}>Run now</Btn>
        <Btn ghost icon="edit" onClick={() => onEdit(routine.id)}>Edit</Btn>
      ...
      <div className="detail-grid">
        <div className="panel">
          ...
          <div className="prompt-block mono"><span className="prompt-mark">❯</span>{routine.prompt}</div>
          <div className="kv-grid">
            <div className="kv"><span className="kv-k mono">working dir</span><span className="kv-v mono"><Icon name="folder" size={13} /> {routine.dir}</span></div>
            <div className="kv"><span className="kv-k mono">model</span><span className="kv-v"><ModelChip model={routine.model} /></span></div>
            <div className="kv"><span className="kv-k mono">next run</span><span className="kv-v mono">{next ? `${routinesUtil.fmtDateTime(next)} (${routinesUtil.relUntil(next, now)})` : '—'}</span></div>
          </div>
        </div>
        <div className="panel">
          <div className="panel-label mono">last 30 days</div>
          <div className="stat-row">
            <div className="stat"><div className="stat-n">{myRuns.length}</div><div className="stat-l mono">runs</div></div>
            <div className="stat"><div className="stat-n" style={{ color: 'var(--green)' }}>{ok}</div><div className="stat-l mono">succeeded</div></div>
            <div className="stat"><div className="stat-n" style={{ color: fail ? 'var(--red)' : undefined }}>{fail}</div><div className="stat-l mono">failed</div></div>
            <div className="stat"><div className="stat-n">${totalCost.toFixed(0)}</div><div className="stat-l mono">total cost</div></div>
          </div>
          <div className="panel-label mono" style={{ marginTop: 18 }}>danger zone</div>
          {confirmDelete ? (
            <div className="confirm-row">
              <span className="dim" style={{ fontSize: 12.5 }}>Delete this routine and keep its history?</span>
              <Btn danger small onClick={() => onDelete(routine.id)}>Delete</Btn>
              <Btn ghost small onClick={() => setConfirmDelete(false)}>Cancel</Btn>
            </div>
          ) : (
            <Btn ghost small icon="trash" onClick={() => setConfirmDelete(true)}>Delete routine</Btn>
          )}
        </div>
      </div>
      ...
      <div className="panel" style={{ marginTop: 14 }}>
        ...
        {myRuns.length === 0 ? (
          <div className="dim" style={{ padding: '14px 2px', fontSize: 13 }}>This routine hasn't run yet.</div>
        ) : (
          <div className="run-list">
            {myRuns.slice(0, 8).map((run) => (
              <div key={run.id} className="run-row" onClick={() => nav({ screen: 'run', runId: run.id, from: { screen: 'routine', routineId: routine.id } })}>
