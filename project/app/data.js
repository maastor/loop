...
;(function () {
  ...
  function parseNL(str) {
    ...
    if (days.length)
      return { freq: 'weekly', days: days.sort(), time, days_: undefined, everyHours: 0 }
  ...
  const RUN_TEMPLATES = {
    'rt-triage': {
      ...
      changes: [
        [
          { t: 'edit', x: 'triage-notes.md' },
          { t: 'label', x: '14 issues labeled on GitHub' }
        ],
        [
          { t: 'edit', x: 'triage-notes.md' },
          { t: 'label', x: '8 issues labeled on GitHub' }
        ]
      ],
      transcript: (p) => [
        { role: 'user', text: p },
        {
          role: 'tool',
          name: 'Bash',
          arg: 'gh issue list --state open --search "created:>=yesterday" --json number,title,body'
        },
        { role: 'result', text: 'Fetched 14 issues' },
        {
          role: 'assistant',
          text: 'I reviewed all 14 issues opened since yesterday. Three look like genuine bugs — #4821 reports balance display freezing after the 2.92 update, which matches the symptoms in #4809, so I flagged it as a possible regression.'
        },
        { role: 'tool', name: 'Bash', arg: 'gh issue edit 4821 --add-label "bug,regression?"' },
        { role: 'tool', name: 'Write', arg: 'triage-notes.md' },
        {
          role: 'assistant',
          text: 'Done. Summary written to triage-notes.md — one item needs your attention: #4821 (possible regression in 2.92).'
        }
      ]
    },
    'rt-deps': {
      ...
      changes: [
        [
          { t: 'commit', x: 'chore(deps): bump 6 patch versions' },
          { t: 'pr', x: 'PR #2381 opened' }
        ],
        [
          { t: 'commit', x: 'chore(deps): minor bumps' },
          { t: 'pr', x: 'PR #2374 opened' }
        ],
        []
      ],
      transcript: (p) => [
        ...
        {
          role: 'assistant',
          text: 'Six packages have patch updates available and one moderate advisory affects ws@8.17.0. The advisory fix is in 8.17.1, which is within the patch range — including it in the bump set.'
        },
        ...
        { role: 'result', text: '842 passing, 0 failing' },
        {
          role: 'tool',
          name: 'Bash',
          arg: 'gh pr create --title "chore(deps): bump 6 patch versions"'
        },
        {
          role: 'assistant',
          text: 'PR #2381 opened with 6 patch bumps, tests green. No major updates needed a decision this time.'
        }
      ]
    },
    'rt-changelog': {
      ...
      changes: [[{ t: 'edit', x: 'CHANGELOG-draft.md' }]],
      transcript: (p) => [
        { role: 'user', text: p },
        ...
        {
          role: 'assistant',
          text: 'I read through all 38 commits since v2.92.0. Most map cleanly to user-facing changes; internal refactors are excluded. Grouping into Added (4), Changed (7), Fixed (11).'
        },
        { role: 'tool', name: 'Write', arg: 'CHANGELOG-draft.md' },
        {
          role: 'assistant',
          text: 'Draft saved. The wording targets end users — e.g. "Fixed a freeze when switching accounts quickly" rather than commit-speak.'
        }
      ]
    },
    'rt-flaky': {
      ...
      transcript: (p) => [
        ...
        {
          role: 'assistant',
          text: 'Comparing first-attempt failures against retry results: swap-flow.spec.ts failed then passed twice in this window. That brings it to 4 flakes this week — over the threshold.'
        },
        { role: 'tool', name: 'Edit', arg: 'flaky-tests.json' },
        {
          role: 'tool',
          name: 'Bash',
          arg: 'gh issue create --title "Flaky: swap-flow.spec.ts (4 flakes this week)"'
        },
        {
          role: 'assistant',
          text: 'Tally updated and issue #1182 opened with links to the four failing runs.'
        }
      ]
    },
    'rt-docs': {
      ...
      changes: [
        [
          { t: 'edit', x: '5 files updated' },
          { t: 'edit', x: 'link-report.md' }
        ],
        []
      ],
      transcript: (p) => [
        ...
        {
          role: 'assistant',
          text: 'Checked all 212 external links. Five returned permanent redirects with obvious targets — updated in place. Three are dead with no clear replacement; listed in link-report.md for a human call.'
        },
    ...
    for (const r of routines) {
      ...
      for (let i = backDays; i >= 0; i--) {
        ...
        for (const t of scheduleTimesForDay(r.schedule, day)) {
          ...
          const cost = +(
            (tokens / 1000) *
            (r.model === 'opus' ? 0.03 : r.model === 'haiku' ? 0.004 : 0.012)
          ).toFixed(2)
          ...
          runs.push({
            ...
            transcript: failed
              ? [
                  ...
                  {
                    role: 'assistant',
                    text: "The GitHub token has expired, so I can't reach the API. Stopping here without making changes — re-run `gh auth login` and this routine will pick up next cycle."
                  }
                ]
