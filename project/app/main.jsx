...
function App() {
  ...
  const [routines, setRoutines] = React.useState(
    () => (stored && stored.routines) || U.seedRoutines
  )
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({ routines, userRuns, pausedAll }))
    } catch (e) {
      /* ignore */
    }
  ...
  if (view.screen === 'routine') {
    ...
    screen = routine ? (
      <RoutineDetailScreen
        ...
        onRunNow={runNow}
        onEdit={(id) => setEditor({ routineId: id })}
          ...
          <div className="sidebar-foot mono">
            {pausedAll ? (
              <span className="sf-line">
                <StatusDot status="paused" /> all paused
              </span>
            ) : (
              <span className="sf-line">
                {running > 0 ? (
                  ...
                  <span>
                    <StatusDot status="success" /> idle
                  </span>
                )}
              </span>
            )}
            {nextAll && !pausedAll ? (
              <span className="sf-line dim">next {U.relUntil(nextAll, now)}</span>
            ) : null}
          </div>
        </div>
